...
{       
	...
	puts("Welcome to the ChatBot, please enter 'quit' to exit.\nYou can type in your messages when >> is displayed.\nHappy Chatting!\n\n");
	...
	if (argc < 2)
		...
		*((int*) buf) = idSem;
		...
		if (strcmp(buf, "quit\n") == 0)
	         { printf("The other User has quit the chat, thank you for using Chatbot!\n");
	         break;}
		...
		if (strcmp(buf, "quit\n") == 0)
		{
		 printf("You have quit the chat, thank you for using Chatbot!\n");
		break;  
		}    
...
/*Allocates a shared memory segment.** @param  n  Size (in bytes) of chunk to allocate.* @return Id of shared memory chunk.*/
int AllocateSharedMemory(int n)
{
assert(n > 0); 
return shmget(IPC_PRIVATE, n, IPC_CREAT | SHM_R | SHM_W);
}
/*** Maps a shared memory segment onto our address space.** @param  id  Shared memory block to map.* @return Address of mapped block.*/
...
/*** Creates a new semaphore set.** @param  n     Number of semaphores in set.* @param  vals  Default values to start off with.* @return Id of semaphore set.*/
int CreateSemaphoreSet(int n, short* vals)
{
union semun arg;
int id;
...
/*** Frees up the given semaphore set.** @param  id  Id of the semaphore group.*/
void DeleteSemaphoreSet(int id)
{
...
/*** Locks a semaphore within a semaphore set.** @param  id  Semaphore set it belongs to.* @param  i   Actual semaphore to lock.** @note If it’s already locked, you’re put to sleep.*/
void LockSemaphore(int id, int i)
{
struct sembuf sb;
sb.sem_num = i;
...
/*** Unlocks a semaphore within a semaphore set.** @param  id  Semaphore set it belongs to.* @param  i   Actual semaphore to unlock.*/
void UnlockSemaphore(int id, int i)
{
struct sembuf sb;
sb.sem_num = i;
